...
After the clouddriver has started you can run below curl

`curl -X GET --header 'Accept: application/json' 'http://localhost:7002/credentials'` and that should return the AWS accounts that were added to the clouddriver.

And now let's attempt to enable AWS lambda for clouddriver.

So, in my understanding one way to edit the clouddriver config is to edit clouddriver/halconfig/clouddriver.yml
...
in ~/.hal directory there was a config file. I've tried to overwrite it for lambda. I've restarted hal and clouddriver, 
that however didn't help. It has still returned null response.

There was another clouddriver file generated by hal in ~/.hal/default/staging an I've edited that file as well. Even after that the lambda api was returning a null response.
